...
If the GPU does NOT ignore the address, it has to divide it into 2 parts high [14:8] and low [7:0] \(note that 15th bit - the most significant one - is always 0, so it cannot be used to identify position).
...
## Reading
`in 1, Reg` must always return GPU type, currently there are 2 types.

## GPU Type
0 - invalid  
1 - minimal (black and white; does not support any commands)   
2 - full 
...
x00YY - invalid
x01YY - invalid
x02YY - set foreground color to YY 
x03YY - set background color to YY 
